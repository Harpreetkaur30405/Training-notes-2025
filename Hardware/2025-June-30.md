...
-       Desktop: Files should be on drive. By default files on desktop are stored on C drive. As Os system is installed in C: file, its recommend to put many folders in desktop. Move them into drives.
...
-       Temporary files: These files are created by websites & cookies, and stored/ download in hidden files. Ultimately because of it the system slows down. So, after a period of time remove temporary files.

-       RAM overuse: Don't open a number of applications or tabs are open at once. As ultimately we are using the RAM.
